...
def flat_Gaussian_beam_beam(sigmax, sigmay, xlist, ylist, order=1):
    r=sigmay/sigmax*order;

    sqrtpi=np.sqrt(np.pi)
    xtilde=xlist/np.sqrt(2.0)/sigmax
    ytilde = ylist / np.sqrt(2.0) / sigmay
    xt2= xtilde*xtilde
    yt2 = ytilde * ytilde
    xy=xtilde*ytilde
    ...
    dawsonx=spefunc.dawsn(xtilde)
    erfy=spefunc.erf(ytilde)

    nEy=expx*erfy-2*r*ytilde/sqrtpi+4*r*xy*dawsonx/sqrtpi
    nEx=2*dawsonx/sqrtpi-2*r*xy*expx*erfy-2*r*xtilde*expx*expy/sqrtpi
    nEx+=r*r*(2*yt2*(dawsonx+xtilde*(1-2*xtilde*dawsonx))+xtilde-xt2*dawsonx)/sqrtpi
